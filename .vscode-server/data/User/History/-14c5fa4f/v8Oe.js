...
const CityAutocomplete = ({ onCitySelect }) => {
  ...
  const customStyles = {
    ...
    menu: (provided) => ({
      ...provided,
      zIndex: 999, // Asegura que el menú esté encima
    }),
  };
  ...
  const handleChange = (option) => {
    ...
    setInputValue(option?.label || ""); // Actualiza el input con el texto seleccionado
    onCitySelect(option?.value); // Devuelve el valor seleccionado al padre
  };

  return (
    <Select
      options={options} // Opciones de la API
      isLoading={loading} // Indicador de carga
      value={selectedOption} // Valor seleccionado
      inputValue={inputValue} // Controla el input temporal
