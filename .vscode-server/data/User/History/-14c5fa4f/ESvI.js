...
const CityAutocomplete = ({ onCitySelect, isAutocompleteEnabled }) => {
  ...
  const fetchCities = async (input) => {
    ...
    try {
      ...
      const cityOptions = response.data.map((city) => ({
        value: city.city, // Envía solo el nombre de la ciudad
  ...
  const handleInputChange = (newValue, { action }) => {
    if (action === "input-change") {
      ...
      if (isAutocompleteEnabled) {
        ...
        onCitySelect(newValue); // Envía el valor directamente al componente padre si el autocompletado está desactivado
      }
    }
  };
  ...
  const handleChange = (option) => {
    if (option) {
      const cityOnly = option.value; // Toma solo el nombre de la ciudad
      setSelectedOption(option); // Actualiza la opción seleccionada
      setInputValue(option.label); // Actualiza el valor del input con la etiqueta completa
  ...
  useEffect(() => {
    if (selectedOption) {
      setInputValue(selectedOption.label); // Asegura que el input muestre la ciudad seleccionada
    }
  }, [selectedOption]);
