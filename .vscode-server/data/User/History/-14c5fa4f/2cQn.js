...
const CityAutocomplete = ({ onCitySelect, isAutocompleteEnabled }) => {
  ...
  const handleInputChange = (newValue, { action }) => {
    if (action === "input-change") {
      ...
      if (isAutocompleteEnabled) {
        debouncedFetchCities(newValue); // Busca ciudades si está habilitado
      } else {
        onCitySelect(newValue); // Envía el valor manualmente
      }
    }
  };
  ...
  // Sincronizar estado cuando se desactiva el autocompletado
  useEffect(() => {
    if (!isAutocompleteEnabled) {
  ...
  return (
    <Select
      ...
      value={selectedOption} // La opción seleccionada
